...
El problema Sim2Real hace referencia a la diferencia entre el rendimiento de una política en el simulador y en el mundo real. Estas diferencias pueden deberse a multiples factores. Puede presentarse por la mala representación de características del entorno o por la percepción del robot, que puede hacer variar las observaciones \cite{da_survey_2025}. Estos y otros factores hacen que sea un problema común para todos los ejercicios de aprendizaje por refuerzo.

En este capítulo se presentan algunas soluciones para el problema. Después, se selecciona aquella que se adapta mejor al trabajo, implementando la solución dentro del ejercicio de entrenamiento. A continuación, se realiza una nueva simulación dentro de IsaacSim, donde no se utilizan las herramientas de IsaacLab. El objetivo es probar la política en un entorno de simulación externo al entrenado. Por último, se realiza la implementación en el robot real. Para ello, se diseña un modulo en python usando herramientas de UR-RTDE \cite{noauthor_universalrobotsrtde_python_client_library_2025}.

\section{Enfoques}
El primer punto que se debe estudiar del problema Sim2Real son los distintos enfoques con los que se puede trabajar para minimizar su efecto. La manera más eficaz de poder adaptar a la implementación real es aleatorizar ciertos parámetros que puedan diferir en el mundo real. De este modo, se puede entrenar la política para afrontarse a variaciones en estos. Por ello, surgen distintos tipos de aprendizaje \cite{sim2realpaper}:
\begin{itemize}
    ...
    \item Curriculum: de nuevo, entrena primero en una simulación ideal e identifica los parámetros a aleatorizar. En este caso, los parámetros se van incluyendo uno a uno, manteniendo los anteriores; finaliza por tanto con un entrenamiento en el que se tienen en cuenta todos los parámetros aleatorizados.
...
Reinforcement Learning for Robotic Manipulation Tasks"} \cite{sim2realpaper} se realizó un estudio de estos distintos tipos de entrenamiento. De este mismo articulo se obtuvo los enfoques anteriores. El que obtuvo el mejor resultado fue el aprendizaje con aleatorización  del dominio, seguido por \emph{fine-tunning}. Debido a ser el mejor modo y tener tiempo limitado dentro del entrenamiento, se escoge esta opción para enfocar el Sim2Real.

En el próximo apartado se estudia como incluir esta herramienta en el aprendizaje, concretamente en el ejercicio de empuje (pues en el de alcance viene integrado en el ejemplo).

\section{Aleatorización del dominio.}
La aleatorización del dominio consiste en proveer a la simulación de una serie de variaciones en el entrenamiento para generalizar la política en el mundo real \cite{tobin_domain_2017}. Para poner un ejemplo de esta implementación se introducen en el ejemplo de empuje una variabilidad en algunas de sus observaciones y eventos.

Dentro de las observaciones, la clase \clase{ObsTerm} permite introducir ruido a la observación mediante el parámetro del constructor \atributo{noise}. Este ruido funciona como la variabilidad del sistema. Se utiliza la clase \clase{GaussianNoiseCfg}, que recibe la siguiente serie de parámetros en su constructor \cite{isaaclab_api}:
\begin{itemize}
    \item \atributo{mean}: la media del ruido. Esto permite incluir un desfase al parámetro. En este caso, la media será 0, pues no se cuenta con ninguna desviación en los medidores.
    \item \atributo{std}: desviación estándar del ruido.
    \item \atributo{operation}: operación para incluir el ruido
\end{itemize}
Este tipo de ruido se incluye en las observaciones de velocidad, de posición del objeto y de posición de la herramienta.

Por otro lado, dentro del manejador de eventos se tienen distintas formas de introducir variabilidad al sistema. Esto se realiza mediante el parámetro \atributo{func}. En este ejemplo se incluyen tres funciones capaces de variar aspectos relevantes:
\begin{itemize}
...
Con esta variabilidad incluida se ha analizado como incluir aleatorización del dominio y preparado el sistema para una posible implementación. A continuación, se estudia como realizar una simulación externa a la herramienta IsaacLab.

\section{Implementación en IsaacSim}
En este apartado se presenta el despliegue de la política dentro del entorno de simulación IsaacSim. La implementación es de desarrollo propio, empleando la clase \clase{PolicyController} y \clase{ConfigLoader} entre otras herramientas de IsaacSim \cite{NVIDIA_IsaacSim}. El código se encuentra dentro del proyecto general, incluido en los anexos. A su vez, este viene inspirado en los ejemplos de IsaacSim, pero se implementa una estructura distinta ya que estos vienen en una extensión sobre la cual no se puede trabajar \cite[isaacsim.examples.interactive]{NVIDIA_IsaacSim}.
...
No obstante, durante la realización de estas pruebas y la realización de nuevos entrenamientos se encontraron nuevos problemas. Uno de estos problemas fue la inefectividad del cierre del gripper. En nuevos entrenamientos se produjeron fallos en el cierre de este, lo que detuvieron el proceso de aprendizaje. Se localizó el problema a dos posibles fuentes: contaminación del archivo .usd al realizar algunas modificaciones o, con mayor probabilidad, errores en el funcionamiento interno de la acción del gripper, concretamente en la clase \clase{BinaryJointActionPosition}. Ambos problemas requieren una mayor dedicación y enfoque en elementos externos al trabajo. Por un lado, funcionamientos internos y muy específicos de IsaacLab, o por el otro, trabajo con el lenguaje .usd. Por esto, se realizo este mismo trabajo con el ejercicio \emph{reach}, que contemplaba desde un primer momento aleatorización del dominio. Confirmado el funcionamiento de su política, se continúa realizando la implementación para este ejercicio. La simulación de este ejercicio se muestra en la carpeta de vídeos de los anexos.
...
En relación con la orientación, las desviaciones obtenidas en los ángulos de rotación se sitúan en torno a 6° para el roll y 5° para el pitch y el yaw, lo que permite considerar la estimación como una aproximación inicial válida. En el caso del yaw se observa un desplazamiento constante cercano a los 135°, tal y como se muestra en la figura \ref{fig:desfasgrip}, por lo que no corresponde a un error del modelo sino a un offset sistemático. Este desfase se debe, por un lado, a las diferencias de convenio empleadas en la definición de los sistemas de referencia y, por otro, a un desfase adicional de aproximadamente 45° asociado a la orientación fija del eje de la herramienta. Una vez considerado este offset, la dispersión del error en yaw es comparable a la obtenida en los otros ejes de rotación.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{imagenes/desfasegrip.png}
    \caption{Desfase en la referencia del gripper.}
    \label{fig:Move}
\end{figure}
